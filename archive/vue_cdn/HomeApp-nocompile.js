// Application Vue pour la page d'accueil (sans compilation de templates)
(function() {
  console.log('🚀 Chargement de HomeApp-nocompile.js...');
  ...
  const app = createApp({
    ...
    // Utiliser une fonction render au lieu d'un template
    render() {
      // Créer un div vide, on utilisera innerHTML dans mounted()
      return h('div', {
        ref: 'container',
        id: 'vue-container'
      });
    ...
    methods: {
      ...
      updateDOM() {
        ...
        const s = this.state;
        let html = '<div class="admin-content-wrapper"><div class="container mt-5">';
        html += '<h2 class="mb-4"><i class="bi bi-house-door me-2"></i>Tableau de bord</h2>';
        ...
        if (s.loading) {
          ...
          // Paniers
          html += `<div class="row mb-4"><div class="col-12 mb-4">
          ...
          if (s.paniersDetails.length === 0) {
            ...
            html += `<div class="table-responsive"><table class="table table-hover mb-0">
              <thead class="table-light">
                <tr><th>ID</th><th>Catalogue</th><th>Articles</th><th>Date d'expiration</th><th>Actions</th></tr>
              </thead><tbody>`;

            s.paniersDetails.forEach(panier => {
              const diffDays = this.calculateDiffDays(panier.expiration_commande);
              const badge = diffDays <= 3 ? `<span class="badge ms-2 ${this.getBadgeClass(diffDays)}">${this.getBadgeText(diffDays)}</span>` : '';
          ...
          if (s.commandesDetails.length === 0) {
            ...
            html += `<div class="table-responsive"><table class="table table-hover mb-0">
              <thead class="table-light">
                <tr><th>ID</th><th>Catalogue</th><th>Articles</th><th>Date de livraison</th><th>Statut</th></tr>
              </thead><tbody>`;

            s.commandesDetails.forEach(commande => {
              html += `<tr>
                <td><span class="badge bg-light text-dark">#${commande.id}</span></td>
                <td>${commande.catalogue_nom}</td>
                <td><span class="badge bg-secondary">${commande.nb_articles}</span></td>
                <td>${commande.livraison}</td>
                <td><span class="badge bg-success"><i class="bi bi-check-circle me-1"></i>${commande.statut}</span></td>
              </tr>`;
            });
              ...
              html += `<div class="col-md-4">
                <div class="card h-100 border">
                  <div class="card-body">
                    ...
                    <a href="/catalogues/${catalogue.id}/vue" class="btn btn-sm btn-primary w-100">
    ...
    mounted() {
      this.updateDOM();
      this.$watch(() => this.state, () => this.updateDOM(), { deep: true });
    }
  });
